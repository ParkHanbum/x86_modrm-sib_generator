...
CALC_RESP calc_modrm_x64(int op1, int op2, effect_t which, int disp, struct MODRM *modrm)
{
	 ...
	 */
	if (disp == disp_none) {
		if (which == OP1 && op1 == disp32)
			op1 -= 5;
		else if (which == OP2 && op2 == disp32)
			op2 -= 5;
	}
	...
	if (op1 < 0 || op2 < 0 || op1 > 7 || op2 > 7 ||
	    ...
	    which < effect_none || which > OP2) {
		fprintf(stderr, "This is not correct value range of arguments!");
		goto FAILED;
	}

	if (which == effect_none) {
		if (disp != disp_none) {
			fprintf(stderr, "disp should be none when effective has none!");
			goto FAILED;
		}
